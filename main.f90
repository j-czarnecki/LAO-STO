PROGRAM MAIN
    ...
    ALLOCATE(Delta(ORBITALS,N_NEIGHBOURS,2))
    ALLOCATE(Delta_new(ORBITALS,N_NEIGHBOURS,2))
    ALLOCATE(Gamma_SC(ORBITALS,N_NEIGHBOURS,2))
    ALLOCATE(Gamma_SC_new(ORBITALS,N_NEIGHBOURS,2))
    ...
    Gamma_SC(:,:,:) = DCMPLX(gamma_start, gamma_start)
    !Gamma_SC(:,:,:) = DCMPLX(0., 0.)
    ...
    DO sc_iter = 1, max_sc_iter
        ...
        !PRINT*, "Gamma = ", Gamma_SC(1,1,1)/meV2au
        counter = 0
        DO i = 0, k1_steps
        ...
        DO spin = 1,2   !Loop over spin coupling up-down or down-up
            DO n = 1, N_NEIGHBOURS !Loop over neighbours
                DO orb = 1, ORBITALS
                    Gamma_SC_new(orb,n,spin) = -0.5*J_SC*Delta_new(orb,n,spin)
                    DO orb_prime = 1, ORBITALS
                        IF(orb .NE. orb_prime) THEN
                            ! CHECK WHETHER THIS IS 0.25 OR 0.5
                            Gamma_SC_new(orb,n,spin) = Gamma_SC_new(orb,n,spin) - 0.25 * J_SC_PRIME * Delta_new(orb_prime, n,spin)
                        END IF
                    END DO
                END DO
            END DO
        ...
        !PRINT*, "Gamma new = ", Gamma_SC_new(1,1,1)/meV2au
        !PRINT*, "Filling ", SUM(Charge_dens(:))/(domega*k1_steps*k2_steps) / DIM_POSITIVE_K
        filling = 0
        filling_total = 0

        WRITE(99,'(I0, 4E15.5)') sc_iter, REAL(Gamma_SC(1,1,1)/meV2au), AIMAG(Gamma_SC(1,1,1)/meV2au), &
        &                                 REAL(Gamma_SC_new(1,1,1)/meV2au), AIMAG(Gamma_SC_new(1,1,1)/meV2au)
        ...
        DO spin = 1, 2
            DO orb = 1, ORBITALS
                DO n = 1, N_NEIGHBOURS
                    ...
                    IF(ABS( ABS(Gamma_SC_new(orb,n,spin)) - ABS(Gamma_SC(orb,n,spin)) )  > eps_convergence) THEN                   
                        sc_flag = .FALSE.
                    EXIT !Maybe go to???
                    END IF
                END DO
        ...
        DO spin = 1, 2
            DO orb = 1, ORBITALS
                DO n = 1, N_NEIGHBOURS
                    Delta_broyden(broyden_index) = REAL(Gamma_SC(orb,n,spin))
                    Delta_broyden(INT(delta_real_elems/2) + broyden_index) = AIMAG(Gamma_SC(orb,n,spin))
                    Delta_new_broyden(broyden_index) = REAL(Gamma_SC_new(orb,n,spin))
                    Delta_new_broyden(INT(delta_real_elems/2) + broyden_index) = AIMAG(Gamma_SC_new(orb,n,spin))
        ...
        DO spin = 1, 2
            DO orb = 1, ORBITALS
                DO n = 1, N_NEIGHBOURS
                    Gamma_SC(orb,n,spin) = DCMPLX(Delta_broyden(broyden_index), Delta_broyden(INT(delta_real_elems/2) + broyden_index))
                    broyden_index = broyden_index + 1
                END DO
            END DO
        END DO
