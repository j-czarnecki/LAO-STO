                        ...
                        DO orb = 1, ORBITALS
                            ...
                            DO band_prime = 1, SUBBANDS
                                IF(band .NE. band_prime) THEN
                                    DO orb_prime = 1, ORBITALS
                                        Gamma_SC_new(orb,n,spin,lat, band) = Gamma_SC_new(orb,n,spin,lat, band) - 0.5 * J_SC_PRIME / 10.0d0 * Delta_new(orb_prime, n,spin,lat, band_prime)
                                    END DO
                                END IF
                            END DO
                    ...
                    DO lat = 1, SUBLATTICES !Loop over sublattices coupling, since for next-to-nearest neighbours it has intra-layer character
                        DO orb = 1, ORBITALS
                            ...
                            DO band_prime = 1, SUBBANDS
                                IF(band .NE. band_prime) THEN
                                    DO orb_prime = 1, ORBITALS
                                        Gamma_SC_new(orb,n,spin,lat, band) = Gamma_SC_new(orb,n,spin,lat, band) - 0.5 * J_SC_PRIME_NNN / 10.0d0 * Delta_new(orb_prime, n,spin,lat, band_prime)
                                    END DO
                                END IF
                            END DO
