...
def runTemperatureDependence():
  ...
  for T in T_table:
    for Ef in Fermi_table:
      t0DirGamma = os.path.join(
        pathToT0, f"RUN_E_Fermi_{Ef[2]}_J_SC_NNN_{J_sc_nnn_nml[2]}", "OutputData", "Gamma_SC_final.dat"
      )
      t0DirCharge = os.path.join(
        pathToT0, f"RUN_E_Fermi_{Ef[2]}_J_SC_NNN_{J_sc_nnn_nml[2]}", "OutputData", "Charge_dens_final.dat"
      )
      if not os.path.exists(t0DirGamma):
        t0DirGamma = os.path.join(
          pathToT0, f"RUN_E_Fermi_{Ef[2]}_J_SC_NNN_{J_sc_nnn_nml[2]}", "OutputData", "Gamma_SC_iter.dat"
        )
        t0DirCharge = os.path.join(
          pathToT0, f"RUN_E_Fermi_{Ef[2]}_J_SC_NNN_{J_sc_nnn_nml[2]}", "OutputData", "Charge_dens_iter.dat"
        )
      ...
      runner.runSlurmParamValue(
        paramValuePairs=[T, Ef, J_sc_nnn_nml, read_gamma, read_charge, gammaDirSetting, chargeDirSetting],
...
def configureAndRunPostprocessing():
  ...
  for dir in directories:
    ...
    runner.runSlurmPostprocessing(dir, [enable_sc, nmlDirectorySC, enable_dos, nmlDirectoryDos], machine="default")
...
def runDosFitting():
  ...
  for Ef in Fermi_table:
    ...
    runner.runSlurmDosFitter(fitConfig=dosFitDict, paramValuePairs=[Ef, B_field], material="KTO", isAres=True)
...
def runMockedOutputPostprocessing():
  ...
  gammaAmplitudesDict = {"nearest": np.complex128(0.0), "next": np.complex128(0.1)}
