...
class DosFitter:
  ...
  def fit(self, paramBounds: list[tuple[float, float]]) -> None:
    ...
    result = differential_evolution(self.__costMse, bounds=paramBounds, disp=True)
  ...
  def __costMse(self, x: list, *args) -> float:
    ...
    DosInterpolated = interpolate(
      self.dosExp["E"]
    )  # Interpolate fited DOS at those points where experimental measurements were made
  ...
  def __getDos(self, x: list) -> pd.DataFrame:
    ...
    gammas = [
      ...
      gammas[1],
      gammas[2],
    ]  # Assume that coupling in two directions is the same and only one is different - as in s-wave from self-consistency
    ...
    result = subprocess.run(
      f"cp {os.path.join(SCRATCH_PATH, self.runsDir, 'OutputData', 'DOS.dat')} {os.path.join(SCRATCH_PATH, self.runsDir, 'DOS_train', f'DOS_{self.nEvals}.dat')}",
      shell=True,
      check=True,
    )
  ...
  def __constructAndWriteGamma(self, gammas: list) -> None:
    ...
    with open(
      f"{os.path.join(SCRATCH_PATH, self.runsDir, 'OutputData', 'Gamma_SC_final.dat')}", "w", newline=""
    ) as file:
