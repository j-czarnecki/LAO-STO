...
class SymmetryResolver(DataReader):
    ...
    def __init__(
        ...
        """
        DataReader.__init__(self, runsPath, matchPattern, sublattices)
                ...
                for orbital in [1, 2, 3]:
                    ...
                    for i in range(len(self.gamma[(1, 1, 1, 1)])):
                        ...
                        for neighbor in range(1, self.nNeighbors + 1):
                            gammaToSymmetrize.append(
                                self.gamma[(spin, neighbor, sublat, orbital)][i]
                            )
                        if i == 0:
                            self.symmetryGammaDict[(spin, sublat, orbital, "s")] = [
                                self._ExtSWavePairing(gammaToSymmetrize)
                            ]
                            self.symmetryGammaDict[(spin, sublat, orbital, "p+")] = [
                                self._PPlusWavePairing(gammaToSymmetrize)
                            ]
                            self.symmetryGammaDict[(spin, sublat, orbital, "d+")] = [
                                self._DPlusWavePairing(gammaToSymmetrize)
                            ]
                        else:
                            self.symmetryGammaDict[(spin, sublat, orbital, "s")].append(
                                self._ExtSWavePairing(gammaToSymmetrize)
                            )
                            self.symmetryGammaDict[
                                (spin, sublat, orbital, "p+")
                            ].append(self._PPlusWavePairing(gammaToSymmetrize))
                            self.symmetryGammaDict[
                                (spin, sublat, orbital, "d+")
                            ].append(self._DPlusWavePairing(gammaToSymmetrize))
                        ...
                        if self.nNextNeighbors != 0:
                            for neighbor in range(
                                ...
                                nnnGammaToSymmetrize.append(
                                    self.gamma[(spin, neighbor, sublat, orbital)][i]
                                )
                            if i == 0:
                                self.nnnSymmetryGammaDict[
                                    (spin, sublat, orbital, "s")
                                ...
                                self.nnnSymmetryGammaDict[
                                    (spin, sublat, orbital, "s")
                                ].append(self._ExtSWavePairing(nnnGammaToSymmetrize))
                                self.nnnSymmetryGammaDict[
                                    (spin, sublat, orbital, "p+")
                                ].append(self._PPlusWavePairing(nnnGammaToSymmetrize))
                                self.nnnSymmetryGammaDict[
                                    (spin, sublat, orbital, "d+")
                                ].append(self._DPlusWavePairing(nnnGammaToSymmetrize))
                                self.nnnSymmetryGammaDict[
                                    (spin, sublat, orbital, "f")
                                ].append(self._FWavePairing(nnnGammaToSymmetrize))
                                self.nnnSymmetryGammaDict[
                                    (spin, sublat, orbital, "d-")
                                ].append(self._DMinusWavePairing(nnnGammaToSymmetrize))
                                self.nnnSymmetryGammaDict[
                                    (spin, sublat, orbital, "p-")
                                ].append(self._PMinusWavePairing(nnnGammaToSymmetrize))
